...
# 2.5D Patches
def gen_patches_batch_augmented_25d_label_indexes_one_hot(image, label, label_indexes, batch_size=32, z_slices=3):
    # valid z_slices is odd
    z_d2 = z_slices//2
    n_label = label[0].shape[-1]
    label_indexes_iter = []
    for d in range(len(label_indexes)):
        label_indexes_iter.append(itertools.cycle(label_indexes[d]))
    ...
    while True:
        for i in range(batch_size):
            # 20% of random patches
            if randint(0,9) < 2:
                d = randint(0, len(image)-1)
                x = randint(0, image[d].shape[2] - PATCH_SIZE - 1)
                y = randint(0, image[d].shape[1] - PATCH_SIZE - 1)
                z = randint(z_d2, image[d].shape[0] - 1 - z_d2)
            else:
                d = randint(0, len(label_indexes)-1)

                z, y, x = next(label_indexes_iter[d])
                y = max(0, y - 1 - (PATCH_SIZE // 2))
                x = max(0, x - 1 - (PATCH_SIZE // 2))

                y = min(max(0, y), image[d].shape[1]-PATCH_SIZE)
                x = min(max(0, x), image[d].shape[2]-PATCH_SIZE)
                
                z = max(z_d2, z)
                z = min(z, image[d].shape[0] - 1 - z_d2)
            
            batch_image[i, :, :, z_d2] = image[d][z, y:y + PATCH_SIZE, x:x + PATCH_SIZE]
            for j in range(1, z_d2+1):
                batch_image[i, :, :, z_d2-j] = image[d][z-j, y:y + PATCH_SIZE, x:x + PATCH_SIZE]
                batch_image[i, :, :, z_d2+j] = image[d][z+j, y:y + PATCH_SIZE, x:x + PATCH_SIZE]
                
            batch_label[i, :, :, :] = label[d][z, y:y + PATCH_SIZE, x:x + PATCH_SIZE]
...
def predict_slice_one_hot_25d(image, model, z_slices):
    # valid z_slices is odd
    z_d2 = z_slices//2
    current_image = np.expand_dims(image, -1)
    prediction_stride = PATCH_SIZE // 3
