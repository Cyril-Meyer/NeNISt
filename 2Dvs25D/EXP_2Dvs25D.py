...
if(os.uname()[1] == 'lythandas'):
    ...
    OUTPUT_FOLDER = "/b/home/miv/cmeyer/2Dvs25D/"
...
EXP = int(sys.argv[3])
GROUPS = int(sys.argv[4])

NAME = NAME + "_" + str(PATCH_SIZE) + "-" + str(DATASET) + "-" +  str(EXP) + "-" + str(GROUPS) + "_" + dt
...
# model = models.UNet.UNet(input_shape=(PATCH_SIZE, PATCH_SIZE, EXP), output_classes=4, filters=30, depth=5, conv_per_block=2, padding='same', dropouts=0.50, batch_normalization=True, groups=[GROUPS, GROUPS, GROUPS, GROUPS, 1, 1, 1, 1 ,1])
model = models.UNet.UNet(input_shape=(PATCH_SIZE, PATCH_SIZE, EXP), output_classes=4, filters=30, depth=5, conv_per_block=2, padding='same', dropouts=0.50, batch_normalization=True, groups=GROUPS)
...
for z in range(5, valid_image.shape[0]-5):
    if EXP == 1:
        ...
        size_x = (valid_image.shape[2]//16)*16+16
        pad_x = size_x - valid_image.shape[2]
        size_y = (valid_image.shape[1]//16)*16+16
        pad_y = size_y - valid_image.shape[1]
        
        batch_image = np.zeros((1, size_y, size_x, 1))
        batch_image[0, 0:valid_image.shape[1], 0:valid_image.shape[2], 0] = valid_image[z, :, :]
        
        predicted_slice = np.argmax(model.predict(batch_image)[0, 0:valid_image.shape[1], 0:valid_image.shape[2]], axis=-1)
        ...
        batch_image = np.zeros((1, size_y, size_x, z_d2*2+1))
        batch_image[0, 0:valid_image.shape[1], 0:valid_image.shape[2], z_d2] = valid_image[z, :, :]
        for j in range(1, z_d2+1):
            batch_image[0, 0:valid_image.shape[1], 0:valid_image.shape[2], z_d2-j] = valid_image[z-j, :, :]
            batch_image[0, 0:valid_image.shape[1], 0:valid_image.shape[2], z_d2+j] = valid_image[z+j, :, :]
...
for z in range(5, test_image.shape[0]-5):
    if EXP == 1:
        ...
        size_x = (test_image.shape[2]//16)*16+16
        pad_x = size_x - test_image.shape[2]
        size_y = (test_image.shape[1]//16)*16+16
        pad_y = size_y - test_image.shape[1]
        
        batch_image = np.zeros((1, size_y, size_x, 1))
        batch_image[0, 0:test_image.shape[1], 0:test_image.shape[2], 0] = test_image[z, :, :]
        
        predicted_slice = np.argmax(model.predict(batch_image)[0, 0:test_image.shape[1], 0:test_image.shape[2]], axis=-1)
    else:
        z_d2 = EXP//2
        ...
        batch_image = np.zeros((1, size_y, size_x, z_d2*2+1))
        batch_image[0, 0:test_image.shape[1], 0:test_image.shape[2], z_d2] = test_image[z, :, :]
        for j in range(1, z_d2+1):
            batch_image[0, 0:test_image.shape[1], 0:test_image.shape[2], z_d2-j] = test_image[z-j, :, :]
            batch_image[0, 0:test_image.shape[1], 0:test_image.shape[2], z_d2+j] = test_image[z+j, :, :]
