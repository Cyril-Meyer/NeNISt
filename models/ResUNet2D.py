...
def conv_block(input, filters, kernel_size=3, conv_per_block=2, dropout=0, batch_normalization=False, groups=1):
    ...
    # save res connection
    RES = Conv2D(filters=filters, kernel_size=1, padding='same', use_bias=False)(X)

    for i in range(conv_per_block):
        X = Conv2D(filters=filters, kernel_size=kernel_size, activation='relu',
                   kernel_initializer="he_normal", padding='same', groups=groups)(X)
...
def RESUNet(input_shape=(None, None, 1), output_classes=2, output_activation='default',
